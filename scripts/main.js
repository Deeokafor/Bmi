...
'use strict';
 ...
 window.onload = function(){
    ...
    submit.addEventListener('click', function startCalc(/*e*/){
       // e.preventDefault();
    ...
    function getKilograms(){
        var open = document.getElementById('output-container');
        open.style.display = 'block';
        var kgms = weight.value / (height.value * height.value);
        result.innerHTML = 'Your BMi is ' + kgms.toPrecision(3) * 100;
        return kgms;
    }
...
//code to check returned result and change color of results div as required
 function resultCorrespond(){
        if(kgms <= 18.5){
            ...
            result.style.backgroundColor = bmiColorChange[3];
        }else{
